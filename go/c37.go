...
func carol(con chan []byte, fin chan int, email string) {
	...
	// Since we set A := k*N, k = 0, 1, 2, ...; Steve computes S as 0
	S := big.NewInt(0)
...
func main() {
	...
	fin := make(chan int)
	go carol(con_ab, fin, "john@mail.com")
	go steve(con_ab, fin)
